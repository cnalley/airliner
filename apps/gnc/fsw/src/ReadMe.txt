...
#
# File:  ReadMe.txt
# Desc:  Description of the GNC cFS application
#
#######################################################################################
...
1) GNC_WAKEUP_MID
   ...
   it sends out its output data (GNC_OUT_DATA_MID) before going back to the
   listening mode.
...
2) GNC_SEND_HK_MID
   ...
   message, the application sends out its latest housekepping data (GNC_HK_TLM_MID)
   ...
   GNC_SEND_HK_MID message.  It should contain ONLY essential data that the ground
...
the GNC_WAKEUP_MID and GNC_SEND_HK_MID commands over other
messages.
