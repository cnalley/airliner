...
#
# File:  ReadMe.txt
# Desc:  Description of the CAMIO cFS application
#
#######################################################################################
...
1) CAMIO_ACQUIRE_IMAGE_MID
   ...
   it sends out its output data (CAMIO_OUT_DATA_MID) before going back to the
   listening mode.
...
2) CAMIO_SEND_HK_MID
   ...
   message, the application sends out its latest housekepping data (CAMIO_HK_TLM_MID)
   ...
   CAMIO_SEND_HK_MID message.  It should contain ONLY essential data that the ground
...
the CAMIO_ACQUIRE_IMAGE_MID and CAMIO_SEND_HK_MID commands over other
messages.
